...
/**
 * This goal causes the mob to stare at the target LivingEntity type infinitely until the
 *  target is nullified, dead, or out of range. Based off of LookAtTargetGoal.
 */
public class ModStareAtEntityGoal extends Goal {
    ...
    public ModStareAtEntityGoal(MobEntity mob, Class<? extends LivingEntity> targetType, float range) {
        ...
        if (targetType == PlayerEntity.class) {
            targetPredicate = TargetPredicate.createNonAttackable().setBaseMaxDistance(range).setPredicate(
                    (entity) -> EntityPredicates.rides(mob).test(entity));
        } else {
    ...
    public void tick() {
        // Look at target each tick as long as the target is non-null and still alive.
